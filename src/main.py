...
class Game:
    ...
    def load_graphics(): #we use it because you don't want global variables as I understood
        ...
        instance.set_images([instance.height()//3*2], Game.path("allez.png"))
        bar_height = 200
        bar = [[pygame.Surface((320, bar_height)), pygame.Surface((340, bar_height)), pygame.Surface((117, bar_height))] for i in range(2)]
        for b in range(len(bar)):
            lastwidth = 0
            for part in range(len(bar[b])):
                pathname = Game.path("barre_d'état_%s.png" % ("g" if b % 2 == 0 else "n"))
                bar[b][part].blit(pygame.image.load(pathname).convert_alpha(), (-lastwidth,0))
                lastwidth += bar[b][path].get_width()
                instance.graphics.update(pathname[:-4]+("%d.png" % part), instance.scale(bar[b][part],
                                                                                         instance.height()//8))
        instance.startmenu_img = instance.scale(pygame.load.image(Game.path("chevreuil_morte.jpg")).convert(), instance.height())
        img = pygame.image.load(Game.path("premier_écran.png")).convert()
        instance.startmenu2_img = instance.scale(img, img.get_height//2)
        img = pygame.image.load(Game.path("terminé.png")).convert()
        instance.gameover_img = instance.scale(img, img.get_height()//4*3)
        instance.control_img = pygame.load.image(Game.path("tableau_de_contrôle.png")).convert()
    ...
    def set_street_img(self, path, path2=None):
        ...
        self.scaled_street = Game.scale(img, img.get_height())
        self.scaled_street2 = Game.scale(img2, img2.get_height())
...
#        return img
    ...
    def scale(img, rel_height):
        return pygame.transform.scale(img, (img.get_width()//img.get_height()*rel_height, rel_height))
    ...
    def scale_list(imgs, rel_heights=-1):
        ...
        if rel_heights == -1 or (not isinstance(rel_heights, list) and not isinstance(rel_heights, tuple)):
            rel_heights = [instance.__height for i in range(len(imgs))]
        lst = len(imgs) if not len(imgs) > len(rel_heights) else len(rel_heights)
        widths = [int(round(float(imgs[i].get_width()) * rel_heights[i] /
                            imgs[i].get_height())) for i in range(lst)]
    ...
    def set_images(self, heights, *paths):
        ...
        self.graphics = dict(zip(paths, [pygame.transform.scale(imgs[i], (imgs[i].get_width(), heights[i]))
...
class Player:
    ...
    def up(self):
        """Moves the Player up."""
        if self.hitbox().top > instance.screen().top+instance.bounds[0]:
            instance.car_speed -= self.speed_y if not self.boost else self.speed_y*self.boost_factor
            self.y -= instance.car_speed

    def down(self):
        """Moves the Player down."""
        if self.hitbox().bottom < instance.screen().bottom+instance.bounds[1]:
            instance.car_speed += self.speed_y if not self.boost else self.speed_y*self.boost_factor
            self.x += instance.car_speed

    def accelerate(self):
        """Accelerates the Player."""
        if self.hitbox().right < instance.screen().right+instance.bounds[3]:
#            print "ACC"
            instance.car_speed += self.speed_x if not self.boost else self.speed_y*self.boost_factor
            self.x += instance.car_speed
#        else:
#            print self.hitbox().right, instance.screen().right

    def decelerate(self):
        """Decelerates the Player."""
        if self.hitbox().left > instance.screen().left+instance.bounds[2]:
#            print "DEC"
